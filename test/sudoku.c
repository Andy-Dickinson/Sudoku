...
srand(1);//////////////////////////////////////////////////////////////////////////
    ...
    solution_grid = malloc(SIZE * sizeof * solution_grid);
    if (solution_grid == NULL) {
        printf("memory allocation failure\n");
        exit(1);
    }
    solution_grid[0] = malloc(SIZE * SIZE * sizeof * solution_grid[0]);
    if (solution_grid[0] == NULL) {
        printf("memory allocation failure\n");
        exit(1);
    }
    for (int i = 1; i < SIZE; i++) {
        solution_grid[i] = &(solution_grid[0][i * SIZE]);
    }

    player_grid = malloc(SIZE * sizeof * player_grid);
    if (player_grid == NULL) {
        printf("memory allocation failure\n");
        exit(1);
    }
    player_grid[0] = malloc(SIZE * SIZE * sizeof * player_grid[0]);
    if (player_grid[0] == NULL) {
        printf("memory allocation failure\n");
        exit(1);
    }
    for (int i = 1; i < SIZE; i++) {
        player_grid[i] = &(player_grid[0][i * SIZE]);
    }
